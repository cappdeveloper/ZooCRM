...
<div class="<?php if($openEdit == true){echo 'open-edit ';} ?>lead-wrapper" <?php if(isset($lead) && ($lead->junk == 1 || $lead->lost == 1)){ echo 'lead-is-junk-or-lost';} ?>>
   ...
   <?php echo form_open('admin/leads/get_lead_excel_export');?>
     ...
     <input type="hidden" name="company" value="<?php echo $lead->company;?>">
     <input type="hidden" name="phone" value="<?php echo $lead->phonenumber;?>">
     <input type="hidden" name="city" value="<?php echo $lead->city;?>">
     <input type="hidden" name="state" value="<?php echo $lead->state;?>">
     <input type="hidden" name="country" value="<?php echo $lead->country;?>">
     <input type="hidden" name="zip" value="<?php echo $lead->zip;?>">
     <input type="hidden" name="address" value="<?php echo $lead->address;?>">
     <input type="hidden" name="status_name" value="<?php echo $lead->status_name;?>">
     <input type="hidden" name="assigned" value="<?php echo $lead->assigned;?>">
...
<input type="submit" name="submit" value="Download Excel" class="btn btn-primary" id="custom_css_submit">
   ...
   <!-- <a href="#" data-toggle="tooltip" id="excel_export" data-title="<?php echo $convert_to_client_tooltip_email_exists; ?>" class="btn btn-primary pull-right lead-convert-to-customer lead-top-btn lead-view" onclick="//convert_lead_to_customer(<?php echo $lead->id; ?>); return false;" lead_name="<?php echo $lead->name;?>" position=" <?php echo $lead->email;?>" email="<?php echo $lead->email;?>" company="<?php echo $lead->company;?>" phone="<?php echo $lead->phonenumber;?>" address="<?php echo $lead->address;?>" country="<?php echo $lead->country;?>" city="<?php echo $lead->city;?>" state="<?php echo $lead->state;?>"  zipcode="<?php echo $lead->zip;?>" >
            <i class="fa fa-user-o"></i>
            Download Excel
      </a> -->
    ...
    var lead_name= $(this).attr('lead_name');
    ...
    var zipcode= $(this).attr('zipcode');
         $.post("<?php echo base_url();?>admin/leads/get_lead_excel_export"/*'http://localhost/CRMwork-master/admin/Npi_api/ajax_get_response'*/, { lead_name: lead_name, position : position,email:email,phone:phone,company:company,address:address,state:state,zipcode:zipcode,address:address,city:city}, 
